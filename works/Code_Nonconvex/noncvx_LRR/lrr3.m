function [X,E,obj,err,iter,Err] = lrr3(A,B,lambda,opts,par)
% nonconvex LRR adapted from Lib-ADMM
...
iter = 0; Y1_hat = Y1; Y2_hat = Y2; c = 10; eta = 0.99;
for iter = 1 : max_iter
    Xk = X;
    Ek = E;
    Jk = J;
    % first super block {J,E}
    [J,nuclearnormJ] = prox_nuclear2(X+Y2/mu,1/mu,par);
    if strcmp(loss,'l1')
    ...
    X = invBtBI*(B'*(Y1_hat/mu-E)+BtA-Y2/mu+J);
