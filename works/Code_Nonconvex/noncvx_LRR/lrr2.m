function [X,E,obj,err,iter,Err] = lrr2(A,B,lambda,opts)
% convex LRR+DM adapted from Lib-ADMM
...
iter = 0; alpha=1; Y1_hat = Y1; Y2_hat = Y2; c = 10;
for iter = 1 : max_iter
    Xk = X;
    Ek = E;
    Jk = J;
    % first super block {J,E}
    [J,nuclearnormJ] = prox_nuclear(X+Y2_hat/mu,1/mu);
    if strcmp(loss,'l1')
    ...
    Xk=X;
    X_hat = X+((alphak-1)/alpha)*(X-Xk);
    Y1_hat = Y1+((alphak-1)/alpha)*(Y1-Y1k); Y2_hat = Y2+((alphak-1)/alpha)*(Y2-Y2k);
    %}
    
      alphak = alpha;
      alpha = (1+sqrt(1+4*alpha^2))/2;
     % X_hat = X+(1-(alphak-1)/alpha)*(X-Xk);
